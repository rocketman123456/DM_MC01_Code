...
void SystemInit(void)
{
  ...
  uint32_t delay_count = 0xFFFF;  //添加延时 等待时钟稳定
  while(delay_count--)
  {
    __NOP();
  }
...
uint8_t SetSysClock_PLL_HSE(uint8_t bypass)
{
    ...
    RCC_OscInitStruct.PLL.PLLM = 6;             // VCO input clock = 1 MHz (8 MHz / 8)
    RCC_OscInitStruct.PLL.PLLN = 180;           // VCO output clock = 360 MHz (1 MHz * 360)
