...
void pack_cmd(uint8_t* msg, int32_t len, Motor_MIT_Data_t* _dm43_mit_t)
{
...
void unpack_reply(uint8_t* msg, int32_t len, Motor_Inf* leg)
{
    /// unpack ints from can buffer ///
    mtr.id = (msg[0])&0x0F;
    mtr.state = (msg[0])>>4;
    mtr.p_int=(msg[1]<<8)|msg[2];
    mtr.v_int=(msg[3]<<4)|(msg[4]>>4);
    mtr.t_int=((msg[4]&0xF)<<8)|msg[5];
    ...
    //mtr.Tmos = (float)(msg[6]);
    //mtr.Tcoil = (float)(msg[7]);
}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    if(rx_header.StdId == Master_ID)
    {
        ...
        CDC_Transmit_FS((uint8_t*) &mtr, sizeof(Motor_Inf));
    }
}
...
void ctrl_motor(CAN_HandleTypeDef* hcan, uint16_t id, uint8_t* msg) //MIT模式
{
    uint32_t send_mail_box;
	...
	Tx_Header.RTR = CAN_RTR_DATA;
	Tx_Header.DLC = 0x08;
	dm43_can_send_data[0] = msg[0];
    dm43_can_send_data[1] = msg[1];
    dm43_can_send_data[2] = msg[2];
    dm43_can_send_data[3] = msg[3];
    dm43_can_send_data[4] = msg[4];
    dm43_can_send_data[5] = msg[5];
    dm43_can_send_data[6] = msg[6];
    dm43_can_send_data[7] = msg[7];
